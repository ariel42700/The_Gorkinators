...
```{r}
top_digi_ctrs <- merged_data %>%
  arrange(better_region, desc(payment_digital)) %>%
  group_by(year, better_region) %>%
  slice_head(n = 5)

top_digi_ctrs %>%
  plot_ly(x = ~better_region, y = ~payment_digital, color = ~country, type = "bar") %>%
  layout(
    ...
    updatemenus = list(
      list(
        active = 0,
        buttons = lapply(unique(top_digi_ctrs$year), function(year) {
          list(
            label = year,
            method = "update",
...
ui <- fluidPage(
  sliderInput("year_slider", "Select Year", min = min(top_digi_ctrs$year), max = max(top_digi_ctrs$year), value = min(top_digi_ctrs$year), step = 1),
...
top_digi_ctrs <- merged_data %>%
  filter(year == 2014) %>%
  arrange(better_region, desc(payment_digital)) %>%
  group_by(better_region) %>%
  slice_head(n = 5)

# Create Plotly plot
plot_top_digi <- plot_ly(top_digi_ctrs, x = ~better_region, y = ~payment_digital, color = ~country, type = "bar") %>%
  layout(
    title = "Top 5 Most Digital Countries in Each Region - 2014",
    xaxis = list(title = "Region"),
    yaxis = list(title = "Payment Digital"),
    barmode = 'stack'
  )
