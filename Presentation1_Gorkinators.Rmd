...
# Create the heatmap using plot_ly with mean digital payments
heatmap <- plot_ly(heatmap_data, x = ~better_region, y = ~group_income, z = ~mean_payment_digital, 
                   type = "heatmap", colors = "Blues",
                   hoverinfo = "x+y+z") %>%
  layout(title = "Mean Digital Payments by Region and Income Group",
...
server <- function(input, output) {
  ...
  output$heatmap <- renderPlotly({
    plot_ly(filtered_data(), x = ~better_region, y = ~group_income, z = ~mean_payment_digital, 
            type = "heatmap", colors = "Blues",
            hoverinfo = "x+y+z") %>%
